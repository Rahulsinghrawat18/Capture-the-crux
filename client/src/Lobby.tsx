...
// Monad local network settings
const contractAddress = '0x8bD9acAA603f038170a63bAAAd076448Abe866A2'; // Default MONAD first address
// GMONAD - 0x44A8d4f0ea6b36FF20C3A98D315b6aFb59C34294
const MONAD_NETWORK = {
  chainId: '0x279F', // 1114 in hex
  chainName: 'Monad Testnet',
  rpcUrls: ['https://testnet-rpc.monad.xyz/'],  // Default Monad URL
};
...
function Lobby({ socket }: LobbyProps) {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      // Check if we're on MONAD network, if not, try to switch
      ...
      // Connect to local MONAD provider
     ...
     // checkShapeKey(selectedAccount);
      ...
      setError('Failed to connect wallet. Make sure your MONAD node is running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="arena-container" style={{ backgroundImage: `url(${LobbyBg})` }}> {/* Dynamic background */}
      <h1 className="arena-title medievalsharp-bold">Capture The Crux</h1>
  ...
  {account ? 'Connected to MONAD Blockchain Testnet' : 'Not Connected'}
  {account && (
    <h1>
...
>
  Create a Room
    ...
    active:translate-y-1 active:shadow-md" 
        disabled={loading}>
          {loading ? 'Connecting...' : 'Connect to MONAD'}
        </button>
      ) : (
        <button className='text-white bg-gradient-to-r 
    from-purple-500 via-purple-600 to-purple-700 
